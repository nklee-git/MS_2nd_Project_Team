...
> **⚠️ 2026-09-10 전환 확정**: 프론트를 **React → ASP.NET Core MVC/Razor Pages**로 전환하기로 확정했습니다. 아래 설명과 `src/` 이하 코드는 v0.5.0까지의 **React 구현(레거시)** 기준입니다 — 화면 구조·데이터 계약(`26. 대시보드 기술명세`)은 그대로 유효하니 Razor Pages로 재구현할 때 참고하되, 실행 명령(`npm run dev` 등)과 기술 스택 설명은 더 이상 최신이 아닙니다.
