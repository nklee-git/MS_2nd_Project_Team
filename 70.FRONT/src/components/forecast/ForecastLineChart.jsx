    ...
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        {skuMode ? (
          <button
            onClick={onClearSku}
            className="inline-flex items-center gap-1 rounded-md border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium text-[var(--color-text-muted)] hover:bg-[#F9F9F9]"
          >
            <X className="h-3 w-3" />
            SKU 선택 해제
          </button>
        ) : (
          <div className="flex flex-wrap items-center gap-1 rounded-lg bg-[#F9F9F9] p-1">
            <button
