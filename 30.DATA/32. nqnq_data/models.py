...
class Inventory(Base):
    ...
    location_id는 STORE.store_id에 대한 정식 FK — "HUB"도 STORE 테이블에
    실제 행으로 존재하는 가상 매장(중앙창고)이라 FK 제약을 정상적으로 건다.
    한 SKU가 여러 location에 각각 재고를 가짐."""
    __tablename__ = "inventory"
    sku_code = Column(String, ForeignKey("sku.sku_code"), primary_key=True)
    location_id = Column(String, ForeignKey("store.store_id"), primary_key=True, default="HUB")
...
class InventoryLedger(Base):
    ...
    sku_code = Column(String, ForeignKey("sku.sku_code"), nullable=False)
    location_id = Column(String, ForeignKey("store.store_id"), nullable=False, default="HUB")  # 2026-09-11 v5 추가 — 어느 위치의 재고가 움직였는지
