...
Dataverse 적재용 CSV를 "샘플 없이 전체" 뽑는 스크립트. 14개 엔터티 전체.

export_for_dataverse.py(샘플판)와 다르게 orders/order_item/customer/
return_request/inventory_ledger도 전부 원본 그대로 뽑음 — 그만큼 파일이
커서(orders/order_item/inventory_ledger는 각 100MB대) 배치 단위로 스트리밍
방식으로 씀(전체를 한 번에 메모리에 올리지 않음).

출력 위치: dataverse_import_full/
"""
import csv
import os
import sqlite3
...
export_table("inventory", ["sku_code", "available_qty", "reserved_qty", "defective_qty",
...
export_table("inventory_ledger", ["ledger_id", "sku_code", "movement_type", "qty_change", "reference_id", "movement_date"], "inventory_ledger.csv")

con.close()
print(f"\n[완료] {OUT_DIR}/ 에 14개 엔터티 전체(샘플 없음) CSV 생성 완료 (nqnq.db 2026-09-11 재생성판 기준)")
print("주의: 큰 파일(orders/order_item/inventory_ledger 등)은 100MB를 넘을 수 있어 GitHub에는 못 올림 - .gitignore 처리 필요")
