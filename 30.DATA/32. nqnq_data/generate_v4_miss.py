...
DB_PATH = "nqnq_scenario_miss.db"
SCENARIO = "miss"

DISRUPTION_WINDOWS = [
    (date(2024, 9, 1), date(2024, 11, 30), 25, 0.85, "Y1 FW 물류 파트너 온보딩 지연"),
    (date(2025, 9, 1), date(2025, 11, 30), 28, 0.83, "Y2 FW 원단 수급 이슈(글로벌 공급망 차질)"),
    (date(2026, 4, 1), date(2026, 8, 9), 45, 0.85, "Y3 여름 시즌 공장 케파 초과(생산 병목)"),
]


def disruption_for(d: date):
    for start, end, extra_delay, qty_ratio, _ in DISRUPTION_WINDOWS:
        if start <= d <= end:
            return extra_delay, qty_ratio
    return None
...
TIER_WEIGHT = {"HERO": 3.0, "STEADY": 1.0, "NICHE": 0.3}  # 2026.08 추가: 롱테일 반영

# 카테고리별 디자인 목록 (2026.08 확장). ACC는 body_codes 자체가 디자인(아이템 종류)이라 제외.
DESIGNS_BY_CATEGORY = {
    "TOP": [
...
# 카테고리별 평균 티어가중치 산출 -> 개별 SKU의 상대적 인기도 비율(tier_ratio) 계산에 사용
...
for q_start, q_end, target in QUARTERS:
    ...
    for d, c in zip(days, counts):
        n = int(c)
        disruption = disruption_for(d)
        if disruption:
            _, qty_ratio = disruption
            n = max(int(round(n * qty_ratio)), 0)  # 공급망 이슈 구간: 수요를 못 채운 만큼 직접 축소
        daily_order_target[d] = daily_order_target.get(d, 0) + n
...
for sid, s in STORES.items():
    ...
    for sku_code, intro in active_from.items():
        if intro > s["open"] or sku_info[sku_code]["line_type"] == "TREND":
            continue
        bump = max(int(velocity_for_date(s["open"]) * tier_ratio_for(sku_code) * 14), 5)  # 오프라인 2주치 예비물량
...
while d <= TODAY:
    ...
    if n_orders > 0:
        ...
        # 날씨(24절기 보간 기온) x 실측 신장분포 x 인기도 티어(롱테일) 기반 SKU 가중치
        # (기온: 기상청 평년값, 신장분포: 2018 설문 기반 사이즈코리아 정합 비율, 티어: HERO/STEADY/NICHE)
        ...
        for s in active_skus:
            ...
            sku_weights.append(w_cat * w_size * w_tier)
        ...
        for _ in range(n_orders):
            ...
            for sku_code in chosen:
                ...
                # 재발주 트리거 (트렌드캡슐 제외 + 공급망 이슈 구간 지연/물량축소)
                _tr = tier_ratio_for(sku_code)
                if (info["line_type"] == "BASIC"
                        and sku_pool[sku_code]["available"] <= reorder_point_for(d, _tr)
                        and not sku_pool[sku_code]["pending_po"]):
                    order_qty = par_level_for(d, _tr) - sku_pool[sku_code]["available"]
                    extra_delay, qty_ratio = 0, 1.0
                    disruption = disruption_for(d)
                    if disruption:
                        extra_delay, qty_ratio = disruption
                        order_qty = max(int(order_qty * qty_ratio), 1)
                    po_id = gen_id("PO")
                    arrival = d + timedelta(days=LEAD_TIME_DAYS + extra_delay)
                    po_rows.append((po_id, "FAC-001", d.isoformat(), arrival.isoformat(),
                                     "발주" if not disruption else "발주(공급망 지연)"))
...
for sku_code, state in sku_pool.items():
    inv_rows.append((
        ...
        pending_return_by_sku.get(sku_code, 0), safety_stock_for(TODAY, tier_ratio_for(sku_code)), reorder_point_for(TODAY, tier_ratio_for(sku_code)),
...
con.close()
print(f"\n[완료] {DB_PATH} 생성 완료 (시나리오: {SCENARIO})")
