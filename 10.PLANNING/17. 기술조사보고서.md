...
| 대시보드 | **ASP.NET Core MVC/Razor Pages** (2뷰: 승인이력, 예측대조) | Dataverse Web API 조회 | — |
...
`Data Factory(오케스트레이션)` → `Databricks ML 처리(결과를 직접 Dataverse에 적재)` → `Dataverse ReorderRecommendation 레코드 생성` → `Power Automate 트리거` → `Teams Adaptive Card` → `승인 시 Dataverse 상태 업데이트` → `ASP.NET Core MVC 대시보드가 Dataverse 조회` (별도 서빙 레이어 없이 Dataverse 단일 저장소 구조)
...
| 단계 | 조건 | 대응 |
| --- | --- | --- |
| 주의(Warning) | 남은 크레딧 7일 이내 소진 예측 | 나경에게 알림, 클러스터/엔드포인트 방치 점검 |
