...
## 0. 제품 정체성 (재확인)
> **2026-09-01 동기화 완료**: [[17. 기술조사보고서]]·[[12. KickOFF]] 2절과 동일하게 아래 정체성을 "SAP 옆에 붙는 재고 리플레니시먼트 포인트 솔루션"으로 갱신함(카카오스타일 SAP S/4HANA 실사용 확인됨, 2026-08-29). [[00.README]]·`팀원모집.md`·루트 `README.md`도 같은 패스에서 동기화 완료.
...
- **NQNQ**: 우리 제품이 아니라 "지그재그가 만든 가상 PB 브랜드"라는 검증용 시뮬레이션 배경(가상 고객사)일 뿐. 108만 건 가상 주문 데이터를 낸 소스([[OBSIDIAN/2026_MS_DATASCHOOL/20.PROJECT/22.MS_2nd_Project/01.MS_2nd_Project_Team/30.DATA/31. NQNQ_data Frame/00. Home/NQNQ Basic Frame|30.DATA]])
...
| --- | --- | --- | --- |
| 데이터 파이프라인 | Azure Data Factory — Databricks 노트북 오케스트레이션 | (협의 예정) | ML이 학습할 데이터셋이 없으면 다음 단계 자체가 시작 안 됨 |
...
| ML·모델링 | **Databricks** 처리 스크립트(기술 핵심), 예측 결과 산출, risk_score 구현, 클러스터 비용 관리 | (협의 예정) | Dataverse에 적재할 필드(예측값·위험점수)의 형태가 여기서 정해짐 |
| 대시보드·프론트 | React 대시보드 2뷰, (Should) Power BI | (협의 예정) | 같은 이유로 Dataverse 스키마에 의존 — 파이프라인의 마지막 소비자 |
| PM | 크로스커팅 계약 관리, 체크포인트 운영, 블로커 해소, 발표자료 | 본인 | 4개 영역이 서로 안 부딪히는지 계속 확인해야 함 |
